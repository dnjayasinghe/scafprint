...
sub draw(;$)
{
	...
	$this->{-pos} != $this->{-max}) { $blocks -2 }

    # Draw center line
	$this->{-canvasscr}->attron(A_BOLD);
    $this->{-canvasscr}->addstr(0, 0," "x($this->canvaswidth -1)."]")
        unless $this->{-nocenterline};
	
	$this->{-canvasscr}->attroff(A_BOLD);


		#init_pair(10, "red", "green");
...
#stdscr.refresh()
	...
	if ($blocks < (($this->canvaswidth)/3) ){
		$this->{-canvasscr}->attron(COLOR_PAIR(11));
		$this->{-canvasscr}->addstr( 0, 0, "|"x($blocks));
		$this->{-canvasscr}->attroff(COLOR_PAIR(11));
	}
	else {
		$this->{-canvasscr}->attron(COLOR_PAIR(10));
		$this->{-canvasscr}->addstr( 0, 0, "|"x($blocks));
		$this->{-canvasscr}->attroff(COLOR_PAIR(10));

		$this->{-canvasscr}->attron(COLOR_PAIR(11));
		$this->{-canvasscr}->addstr( 0, 0, "|"x(($this->canvaswidth)/3));
		$this->{-canvasscr}->attroff(COLOR_PAIR(11));
    ...
    if (not $this->{-nopercentage} or $this->{-showvalue})
    {
        ...
        my $len = length($str);
        my $xpos = int(($this->canvaswidth - $len -1));
