...
class SmartcardSerial():
    ...
    def enc(self,datai):
        self.APDUSend(0x80,0xC0,0x00,0x00,datai) #datai
        time.sleep(.1)
        data = bytearray(self.ser.read(300))
        buf = ""
        for p in data:
            buf = buf + "%c"%p
        print buf
...
def main():
    ...
    test.connect(port)
    data_tab = open("./pointsBP_10000.txt", "r") 
    for i in range(0):
        datai=data_tab.readline()[:-1]
   
    for i in range(100):
        datai=data_tab.readline()[:-1]
        for j in range(100):
            print test.enc(datai)
